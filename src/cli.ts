        ...
        if (options.json) {
          ...
          if (result.failures.length > 0) {
            ...
            result.failures.forEach((test, i) => {
              ...
              if (test.file_path)
                console.log(
                  `   File: ${test.file_path}${test.line_number ? `:${test.line_number}` : ""}`,
                );
...
program
  ...
  .option(
    "--sort <type>",
    "Sort order (chronological|by_file|by_author|priority)",
  )
  .option(
    "--include-status-indicators",
    "Include status indicators and priority scoring",
  )
    ...
    async (options: {
      ...
      try {
        ...
        if (options.json) {
          ...
          console.log(`\n📊 Summary:`);
          console.log(
            `   Bots: ${result.summary.bot_comments}, Humans: ${result.summary.human_comments}`,
          );
...
program
  ...
  .requiredOption(
    "--dependent-pr <identifier>",
    "Dependent PR (owner/repo#124)",
  )
      ...
      try {
        ...
        if (options.json) {
          ...
          if (result.change_summary) {
            console.log(
              `Changes: ${result.change_summary.new_commits_in_base} new commits in base`,
            );
            console.log(
              `Files changed: ${result.change_summary.files_changed.length}`,
            );
          }
          ...
          if (result.commands.length > 0) {
            ...
            result.commands.forEach((cmd) => {
              ...
              if (cmd.estimated_duration)
                console.log(`   ⏱️  ${cmd.estimated_duration}`);
            });
          }
