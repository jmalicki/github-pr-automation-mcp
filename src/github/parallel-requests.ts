...
export class ParallelRequestHandler {
   ...
   */
  async executeBatch<T>(requests: BatchRequest<T>[]): Promise<BatchResult<T>[]> {
   ...
   */
  private async executeRequest<T>(request: BatchRequest<T>): Promise<BatchResult<T>> {
    try {
   ...
   */
  async batchCheckRuns(commits: Array<{ owner: string; repo: string; sha: string }>): Promise<BatchResult<unknown>[]> {
   ...
   */
  async batchComments(prs: Array<{ owner: string; repo: string; number: number }>): Promise<BatchResult<unknown>[]> {
    const requests: BatchRequest<unknown>[] = prs.map(pr => ({
 ...
 */
export function createParallelHandler(client: GitHubClient, maxConcurrency = 5): ParallelRequestHandler {
