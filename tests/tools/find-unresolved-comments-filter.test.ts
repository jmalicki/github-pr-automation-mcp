...
describe('filterUnresolvedComments', () => {
  const createMockComment = (overrides: Partial<Comment> = {}): Comment => ({
    ...
    action_commands: {
      reply_command: 'gh pr comment 1 --repo test/repo --body "YOUR_RESPONSE_HERE"',
  ...
  it('should include original comments without in_reply_to_id', () => {
    ...
    const result = filterUnresolvedComments(comments, nodeIdMap, resolvedThreadIds);
  ...
  it('should exclude comments from resolved threads', () => {
    const comments = [
      createMockComment({ id: 1, type: 'review_comment', in_reply_to_id: undefined }),
      createMockComment({ id: 2, type: 'review_comment', in_reply_to_id: undefined })
    ];
    const nodeIdMap = new Map<number, string>([
      [1, 'thread-1'],
      [2, 'thread-2']
  ...
  it('should handle mixed comment types correctly', () => {
    const comments = [
      createMockComment({ id: 1, type: 'review_comment', in_reply_to_id: undefined }), // Original review comment
      createMockComment({ id: 2, type: 'review_comment', in_reply_to_id: 1 }), // Reply to review comment
      createMockComment({ id: 3, type: 'issue_comment', in_reply_to_id: undefined }), // Original issue comment
