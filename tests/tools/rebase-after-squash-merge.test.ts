...
describe("handleRebaseAfterSquashMerge", () => {
  ...
  it("should return basic rebase commands with default target branch", async () => {
    ...
    expect(byStep(result.commands, 2).command).toContain(
    ...
    expect(byStep(result.commands, 3).command).toBe(
      "git push --force-with-lease origin feature-branch",
    );
    expect(result.summary.action_required).toBe(true);
    expect(result.summary.reason).toBe(
      "Manual upstream commit identification needed in this phase",
    );
  });
  ...
  it("should provide detailed command descriptions", async () => {
    ...
    expect(result.commands[1].description).toContain(
      "Rebase using --onto to skip squash-merged commits",
    );
  ...
  it("should handle different branch names", async () => {
    ...
    expect(result.commands[1].command).toContain(
      "git rebase --onto origin/develop",
    );
