...
describe('handleCheckMergeReadiness', () => {
  ...
  it('should detect merge conflicts', async () => {
    ...
    expect(result.blocking_issues[0].description).toBe('PR has merge conflicts');
  });
  ...
  it('should handle multiple blocking issues', async () => {
    ...
    expect(result.blocking_issues.some(issue => issue.category === 'ci')).toBe(true);
    expect(result.blocking_issues.some(issue => issue.category === 'conflicts')).toBe(true);
  });
