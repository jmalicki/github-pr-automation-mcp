...
describe("handleFindUnresolvedComments", () => {
  ...
  it("should paginate comments correctly with real GitHub API fixture data", async () => {
    ...
    expect(page3.comments).toHaveLength(
      paginationFixture[0].response.length - 2,
    ); // Remaining items from fixture
...
\`\`\`
          ...
          html_url:
            "https://github.com/test/repo/pull/123#pullrequestreview-123",
        },
      ],
      headers: { link: "" },
    });
    ...
    expect(result.comments[0].body).toContain(
      "Consider copying the lockfile for reproducible builds",
    );
...
\`\`\`
            ...
            html_url:
              "https://github.com/test/repo/pull/123#pullrequestreview-124",
          },
        ],
        headers: { link: "" },
      });
      ...
      expect(result.comments[0].coderabbit_metadata?.suggestion_type).toBe(
        "nit",
      );
      expect(result.comments[0].coderabbit_metadata?.severity).toBe("low");
      expect(result.comments[0].coderabbit_metadata?.file_context?.path).toBe(
        "src/file.ts",
      );
      expect(
        result.comments[0].coderabbit_metadata?.file_context?.line_start,
      ).toBe(36);
      expect(result.comments[0].coderabbit_metadata?.agent_prompt).toContain(
        "CodeRabbit nit suggestion",
      );
    });
...
\`20\`: **Critical security issue that must be fixed**
      ...
      expect(result.comments[1].coderabbit_metadata?.suggestion_type).toBe(
        "nit",
      );
    });
...
\`10\`: **Minor suggestion**
      ...
      expect(result.comments).toHaveLength(1);
      expect(
        result.comments[0].coderabbit_metadata?.agent_prompt,
      ).toBeUndefined();
    });
...
\`10\`: **Minor suggestion**
      ...
      expect(
        result.comments[0].coderabbit_metadata?.agent_prompt,
      ).toBeDefined(); // Should be generated by default
    });
  });

  describe("Status Indicators and Priority Ordering", () => {
    it("should add status indicators when enabled", async () => {
      // Mock review comments response with bot comment
      mockOctokit.pulls.listReviewComments.mockResolvedValue({
        data: [
          {
            id: 1,
      ...
      expect(comment.status_indicators).toBeDefined();
      expect(comment.is_bot).toBe(true);
      expect(comment.status_indicators?.priority_score).toBeGreaterThan(0);
      expect(comment.status_indicators?.resolution_status).toBe("unresolved");
      expect(comment.status_indicators?.suggested_action).toBeDefined();
    });

    it("should implement priority-based sorting", async () => {
      // Mock review comments with different priority levels
      mockOctokit.pulls.listReviewComments.mockResolvedValue({
        data: [
          {
            id: 1,
          ...
          },
          {
            ...
            line: 20,
            diff_hunk: "@@ -18,5 +18,5 @@",
            body: "Critical security vulnerability - fix immediately",
            reactions: {
              total_count: 0,
              "+1": 0,
              "-1": 0,
      ...
      // The security vulnerability should come first (higher priority)
      const firstComment = result.comments[0];
      const secondComment = result.comments[1];

      expect(firstComment.body).toContain("Critical security vulnerability");
      expect(secondComment.body).toContain("Minor style issue");

      // Priority scores should be in descending order
      expect(
        firstComment.status_indicators?.priority_score,
      ).toBeGreaterThanOrEqual(
        secondComment.status_indicators?.priority_score || 0,
      );
    });

    it("should include priority summary in output", async () => {
      // Mock responses
      mockOctokit.pulls.listReviewComments.mockResolvedValue({
        data: [],
        headers: { link: "" },
      ...
      expect(result.summary.priority_summary).toBeDefined();
      expect(result.summary.priority_summary?.high_priority).toBe(0);
      expect(result.summary.priority_summary?.medium_priority).toBe(0);
      expect(result.summary.priority_summary?.low_priority).toBe(0);
    ...
    it("should include status groups when priority ordering is enabled", async () => {
      ...
      expect(result.summary.status_groups).toBeDefined();
      expect(result.summary.status_groups?.unresolved).toEqual([]);
