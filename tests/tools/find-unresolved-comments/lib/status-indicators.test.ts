  ...
  describe('calculateStatusIndicators', () => {
    ...
    it('should reduce priority for outdated comments', () => {
      ...
      if (outdatedIndicators && freshIndicators) {
        expect(outdatedIndicators.priority_score).toBeLessThanOrEqual(freshIndicators.priority_score);
      }
    });
    ...
    it('should detect manual responses and mark as acknowledged/in_progress', () => {
      ...
      const indicatorsWithoutResponse = calculateStatusIndicators(originalComment, []);
      const indicatorsWithResponse = calculateStatusIndicators(originalComment, [replyComment]);
      ...
      expect(indicatorsWithoutResponse?.priority_score).toBeGreaterThan(indicatorsWithResponse?.priority_score || 0);
    });
    ...
    it('should detect actionable content in comment body', () => {
      ...
      const actionableIndicators = calculateStatusIndicators(actionableComment, []);
      const nonActionableIndicators = calculateStatusIndicators(nonActionableComment, []);

      expect(actionableIndicators?.is_actionable).toBe(true);
      expect(nonActionableIndicators?.is_actionable).toBe(false);
      expect(actionableIndicators?.priority_score).toBeGreaterThan(nonActionableIndicators?.priority_score || 0);
    });
    ...
    it('should suggest appropriate action based on comment characteristics', () => {
      ...
      expect(['reply', 'investigate', 'ignore']).toContain(lowPriorityIndicators?.suggested_action);
    });
  });
});
