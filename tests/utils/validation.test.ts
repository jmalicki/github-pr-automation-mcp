...
import { PRIdentifierStringSchema, PaginationSchema } from '../../src/utils/validation.js';
...
describe('validation', () => {
  describe('PRIdentifierStringSchema', () => {
    ...
    it('should validate full GitHub URL format', () => {
      const result = PRIdentifierStringSchema.safeParse('https://github.com/owner/repo/pull/123');
      expect(result.success).toBe(true);
      if (result.success) {
    ...
    it('should reject malformed URL', () => {
      const result = PRIdentifierStringSchema.safeParse('https://github.com/owner/repo/issues/123');
      expect(result.success).toBe(false);
    });
  });
