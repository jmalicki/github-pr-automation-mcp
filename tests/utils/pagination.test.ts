...
describe("Cursor pagination", () => {
  ...
  describe("paginateResults", () => {
    ...
    it("should throw error for invalid page size", () => {
      expect(() => paginateResults(items, undefined, 0)).toThrow(
        "pageSize must be",
      );
      expect(() => paginateResults(items, undefined, -1)).toThrow(
        "pageSize must be",
      );
      expect(() => paginateResults(items, undefined, Infinity)).toThrow(
        "pageSize must be",
      );
    });
