...
describe('Server-side pagination utilities', () => {
  ...
  describe('integration scenarios', () => {
    ...
    it('should handle edge case with very large offsets', () => {
      const cursor = Buffer.from(JSON.stringify({ offset: 1000, pageSize: 20 })).toString('base64');
      const result = cursorToGitHubPagination(cursor, 20);
      expect(result).toEqual({ page: 51, per_page: 20 }); // 1000/20 + 1 = 51
    });

    it('should handle edge case with page size 1', () => {
      const cursor = Buffer.from(JSON.stringify({ offset: 5, pageSize: 1 })).toString('base64');
      const result = cursorToGitHubPagination(cursor, 20);
      expect(result).toEqual({ page: 6, per_page: 1 }); // 5/1 + 1 = 6
    });

    it('should handle edge case with page size 100 (maximum)', () => {
      const cursor = Buffer.from(JSON.stringify({ offset: 200, pageSize: 100 })).toString('base64');
      const result = cursorToGitHubPagination(cursor, 20);
    ...
    it('should throw error for invalid defaultPageSize', () => {
      expect(() => cursorToGitHubPagination(undefined, 0)).toThrow('defaultPageSize must be');
      expect(() => cursorToGitHubPagination(undefined, -1)).toThrow('defaultPageSize must be');
      expect(() => cursorToGitHubPagination(undefined, Infinity)).toThrow('defaultPageSize must be');
    });
    ...
    it('should handle createNextCursor with different page sizes', () => {
      ...
      const decoded100 = JSON.parse(Buffer.from(result100!, 'base64').toString('utf-8'));
    ...
    it('should handle createNextCursor with existing cursor and different page sizes', () => {
      const currentCursor = Buffer.from(JSON.stringify({ offset: 10, pageSize: 5 })).toString('base64');
