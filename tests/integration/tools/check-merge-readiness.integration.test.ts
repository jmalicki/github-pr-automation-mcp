...
describe('check_merge_readiness integration', () => {
  ...
  beforeAll(async () => {
    ...
    const fixture = await integrationManager.loadFixture('check-merge-readiness/basic-pr');
  ...
  it('should identify specific merge blockers', async () => {
    ...
    // Save fixture if in record mode
    await integrationManager.saveFixture('check-merge-readiness/blockers', result);
  }, 10000);
  ...
  it('should provide actionable next steps', async () => {
    ...
    // Save fixture if in record mode
    await integrationManager.saveFixture('check-merge-readiness/next-steps', result);
  }, 10000);
  ...
  it('should validate CI status and branch protection', async () => {
    ...
    const ciChecks = result.checks.filter(check => 
    ...
    const protectionChecks = result.checks.filter(check => 
    ...
    // Save fixture if in record mode
    await integrationManager.saveFixture('check-merge-readiness/ci-protection', result);
  }, 15000);
});
